...
1. Read the inbox (`ofocus tasks --in-inbox`) and, where useful, the decision-ready context (`ofocus today`, `ofocus stalled-projects`, `ofocus next-actions`). When an agent runs the CLI, machine output is TOON by default — no `--format` flag needed.
