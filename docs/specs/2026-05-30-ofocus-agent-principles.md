...
1. **Compute, don't reason.** Deterministic code owns date arithmetic, recurrence
   ...
   _(A1 diffs, A2 recurrence, A3 derived state)_

2. **Resolve, don't dump.** The agent passes _fuzzy references_ ("Project Falcon"); the
   tool returns either a resolved entity or a **tight ranked candidate set** for
   disambiguation — not a wall of rows to reason over. _(A4)_
...
4. **Push state, don't poll-and-diff.** Reads are cache-first and instant; the tool tracks
   _what the agent has already seen_ (a per-watch `generation`) and corrections are pushed
   into the agent's context out of band. The agent does no snapshot bookkeeping.
   _(A1 data + plugin hook)_
...
- _Audience:_ programs and power users who know precisely what they want.
- _Property:_ direct and unsurprising — no inference, no magic.

### Layer 2 — Productivity niceties (still inside the tool)

**Actor-agnostic** conveniences that absorb mechanical work so _anyone_ is more productive —
...
- _Audience:_ humans-at-terminal **and** agents.
- _Litmus test:_ **"Would a human typing a quick command want this, and is the answer
...
co-planning, the disambiguation _conversation_ (using Layer 2's candidate sets), action-item
tracking, and human↔agent handoff conventions.

- _Audience:_ agents (and the human via the agent).
- _Litmus test:_ **"Is this only meaningful inside an agent's runtime, or about
  _delivering / orchestrating_ rather than _computing_?"** → yes ⇒ Layer 3.

### Why the boundary lands where it does

A terminal human wants fuzzy search (L2); only an agent runtime even _has_ a "PreToolUse
...
| **`@ofocus/productivity`** _(new)_ | L2    | The niceties — fuzzy search, anchored create, recurrence expansion, derived state, change detection. | Depends on `@ofocus/sdk`. **Sanctioned home for any extra weight** a nicety needs (still minimal by preference; Node built-ins like `child_process` are fine). |
...
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | -------- | ----------- |
| **F**          | **Typed OmniAutomation authoring foundation** — `defineOmniScript`/`defineOmniAction` compile type-checked TypeScript to OmniJS; `osascript` + headless plugin-install backends; the `@ofocus/omnijs-types` ambient-types package; the `no-omniscript-closure` guardrail. The substrate A3+ are authored on.                                                                                                | **L1** (SDK)              | —        | **Shipped** |
| **A1**         | `changes` primitive — cache-first reads, `generation`, field-level diffs, fingerprint fast path (+ optional Full Disk Access accelerator), `--fresh`, `--semantic`                                                                                                                                                                                                                                          | **L2** (data); hook is L3 | 1, 4     | **Shipped** |
| **A2**         | Temporal engine — recurrence _expansion_ (`next-occurrences`), occurrence forecasting in a window, computed `dueIn` / `overdueBy`, `today` / `this-week` digests                                                                                                                                                                                                                                            | **L2**                    | 1, 3     | **Shipped** |
| **A3**         | Derived-state engine — explicit `effectiveStatus`, `blockedReason` (precedence-ordered array), `isNextAction`, and project `stalled`/`empty`; `stalled-projects` and `next-actions` commands. Raw native fields (`taskStatus`, `availableTaskCount`, effective dates) live in **L1**; the opinionated engine is an enriched wrapping-SDK surface in **L2** — _not_ the original "A3 = L2 field-spec" shape. | **L1**+**L2**             | 1        | **Shipped** |
| **A4**         | Resolution & disambiguation — fuzzy/ranked entity search, the disambiguation response contract, temporal-anchor resolution (built on A2), anchored-create convenience (`--before "1:1 with Sarah"`)                                                                                                                                                                                                         | **L2**                    | 2, 3     | **Shipped** |
| **(calendar)** | Calendar **conversance** — task↔event links + meeting-readiness over **agent-supplied** event snapshots (no calendar read). Originally listed out of scope; shipped via the snapshot boundary.                                                                                                                                                                                                              | **L2**                    | 1, 3     | **Shipped** |
...
**Type-safe at every layer (tenet).** OmniFocus automation is authored as type-checked
TypeScript end-to-end: the CLI and MCP are thin typed adapters over the SDK(s); the SDK is the
programmatic product; and even the OmniJS the SDK emits is authored type-safely via the **F**
foundation rather than concatenated as strings. New OmniJS should be authored on
`defineOmniScript`/`defineOmniAction`, never hand-built as strings.

**What's next.** With **F** and **A1–A4** (and calendar conversance) shipped, the remaining
roadmap item is **B** — the chief-of-staff agent plugin that exercises judgment over L1/L2's
now decision-ready data. (The existing `omnifocus-automation` plugin is an early cut of B.)
...
- **Calendar _reading_ (EventKit) — out of scope; calendar _conversance_ shipped differently.**
  The tool never reads a calendar. **Calendar conversance shipped** via _agent-supplied_ event
  snapshots: the agent passes event data into the `link` / `readiness` commands, and the tool
  computes meeting-readiness deterministically over task↔event links — no EventKit, no calendar
  access. Temporal-anchor resolution (A4) still returns `{ none: true }` rather than guessing
  when an anchor refers to a calendar event the tool itself cannot see.
